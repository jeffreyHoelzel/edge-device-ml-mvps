...
def evaluate_validation(
    ...
    """Return incident, root-cause, and severity metrics for validation data."""
    with torch.no_grad():
...
def train(data_path: Path, model_path: Path, epochs: int = 30, batch_size: int = 64, seed: int = 7) -> RootCauseGRU:
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            model.eval()
            validation_metrics = evaluate_validation(model, features, causes, severity, incident, validation_indices)
            model.train()
            print(f"epoch={epoch + 1:02d} loss={running_loss / len(train_indices):.4f} validation_incident_f1={validation_metrics['incident_f1']:.3f} validation_cause_accuracy={validation_metrics['cause_accuracy']:.3f} validation_severity_accuracy={validation_metrics['severity_accuracy']:.3f}")
